...
- 이클립스에서 톰캣 서버를 싱행하고 종료할 수 있다. 
- 웹 프로젝트를 테스트하기 위해 별도의 테스트 환경을 구축한다.
- 그래서 원본 톰캣 서버에 실행 환경을 손대지 않는다.
...
- 이클립스 워크스페이스에 "Servers"라는 프로젝트 폴더가 생성된다.
    ...
    - 이 폴더에서 톰캣 서버 폴더에서 복사해온 설정 파일들이 들어 있다.
    ...
    - 톰캣 서버 테스트 환경을 실행하면    해당 디렉토리가 구축된다.
    - 이클립스 워크 스페이스 폴더에 톰캣 서버 테스트 환경 디렉토리가 구축된다.
  ...
  .plugins\
     org.eclipse.wst.server.core\
        tmp0\       -톰캣 서버 테스트 환경(생성된 순서대로 0,1,2... 식으로 이름이 부여된다.)
          conf\     - 이클립스 "Servers/테스트 환경 폴더"에 별도로  편집된 설정 파일을 복사해온다.
           logs\    - 테스트 환경에서 톰캣 서버를 실행하는 중에 출력된 로그 파일
           temp\    - 테스트 환경에서 톰캣 서버를 실행하는 중에 임시 작업 파일
           Webapps\ - 테스트 환경에서는 톰캣 서버를 실행할 때는 이 폴더를 사용하지 않는다.
...
- 주의!
   - 서버가 실행 중일 때 베포를 하면 자동으로 배포 파일이 복사된다.
   - 그러면 톰캣 서버는 배포한 웹 애플리케이션을 자동으로 로딩한다.
   - 하지만 가끔 자동 복사가 안될 때가 있다.
   - 그럴 경우 톰캣 서버를  멈추고, 배포 프로젝트를 제거한 후 위의 과정을 다시 수행하라!
- 톰캣 서버 테스트 환경에는 톰캣 서버 원본에 있던 웹 애플리케이션이 없기 때문에 기본으로 웹 페이지를 출력하는 웹 페이지가 없다
...
예)
http://localhost:9999/java110-project/index.html
'''
   

### 애플리케이션 배치명을 변경하기

- 애플리케이션 배치 이름을 지정하지 않으면 기본이 프로젝트 명이다.
...
배치 폴더 (tmp0/wtpwebapps)에 프로젝트를 배치할 때 프로젝트 이름으로 배치 되더라도 웹 브라우저에서 요청할 때는 프로젝트명이 아니라 다음과 같이 배치 이름으로 실행해야 한다.


'''
http://localhost:9999/web01/index.html
'''
...
- 컴포넌트
   - 서블릿, 필터, 리스너
   - 컴포넌트는 배치 폴더에서 /WEB-INF/classes 폴더에 배치한다.
- 웹 애플리케이션 배치 정보 파일
...
- 보통 줄여서 'WAS'라고 표현한다. 그냥 'AS'라고 실행할 때도 있다.

### 구성 요소

- HTTP Server
   - HTTP 프로토콜을 기반으로 웹 브라우저의 요청을 받고 응답하는
   - 보통 실무에서 운영할 때는 웹 애플리케이션 서버에 내장된 HTTP 서버를 사용하지 않고 별도의 HTTP Server(ex: Apache, nginX 등)를 사용한다.  
- Servlet Container
   - JavaEE 스펙 중에서 웹 관련 스펙을 구현한 서버이다. 
   - 서블릿, 필터, 리스너, JSP를 관리하고 실행하는 서버
- EJB Container
   - JavaEE 스펙 중에서 EJB 스펙을 구현한 서버이다.
   - 현재는 EJB를 잘 사용하지 않는다.
...
- 자바로 기업용 애플리케이션을 제작하는 데 필요한 기술을 정의한 명세이다.
- 기술 요소
...
- Java EE 버전
   ...
   - 버전 예:
      ...
      - Java EE 5 : Servlet 2.0, JSP 2.1, JSTL 1.1, EJB 3.0 등   
...
서블릿은 클라이언트의 요청을 들어왔을 때 호출되는 자바 객체이다.
